...
typedef struct CreateStmt
{
	...
	bool	   isarray; 		/*is the table represents a time series or array*/
      					/* used in standard_ProcessUtility */
} CreateStmt;
...
{
	NodeTag		type;
	int			whatToPrint; //0-MODELGRAPH,1-MODELCOUNT
} PrintStmt;
...
typedef struct ForecastExpr
{
	 ...
	 * OR
	 * -1 - forecast to a future-date
	 * -2 - CreateModel-Stmt
	 */
	int 			length;

	//an alternative length field to store the number of units of the given granularity that should be forecasted
	int				altLength;
	...
	short			storeModel;
	int				maintenancetype;
	List			*maintenanceParameters;

	Value			*confidenceIntervalPercentage;
} ForecastExpr;
