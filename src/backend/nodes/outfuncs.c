...
_outForecastExpr(StringInfo str, ForecastExpr *node)
{
	...
	WRITE_INT_FIELD(length);
	WRITE_INT_FIELD(altLength);
	WRITE_INT_FIELD(choose);
	WRITE_STRING_FIELD(targetDateString);
	WRITE_STRING_FIELD(modelName);
	WRITE_INT_FIELD(storeModel);
	WRITE_INT_FIELD(maintenancetype);
	WRITE_NODE_FIELD(maintenanceParameters);
	WRITE_NODE_FIELD(confidenceIntervalPercentage);
