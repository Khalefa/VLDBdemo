...
_readForecastExpr(void)
{
	...
	READ_ENUM_FIELD(granularity,Granularity);
	READ_INT_FIELD(aggType);
	READ_INT_FIELD(length);
	READ_INT_FIELD(altLength);
	READ_INT_FIELD(choose);
	READ_STRING_FIELD(targetDateString);
	READ_STRING_FIELD(modelName);
	READ_INT_FIELD(storeModel);
	READ_INT_FIELD(maintenancetype);
	READ_NODE_FIELD(maintenanceParameters);
	READ_NODE_FIELD(confidenceIntervalPercentage);
