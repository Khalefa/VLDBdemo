...
static SimpleReestMaintenance* _copySimpleReestMaintenance(SimpleReestMaintenance* from)
{
	SimpleReestMaintenance* newnode=makeNode(SimpleReestMaintenance);
	COPY_SCALAR_FIELD(errorReference);
	COPY_SCALAR_FIELD(insertCount);
	COPY_NODE_FIELD(parameter);
	COPY_SCALAR_FIELD(reestInsCnt);
	newnode->initMaintenance=from->initMaintenance;
	newnode->triggerMaintenance=from->triggerMaintenance;
	newnode->insert=from->insert;
	return newnode;
}

static EmptyMaintenance* _copyEmptyMaintenance(EmptyMaintenance* from)
{
	EmptyMaintenance* newnode=makeNode(EmptyMaintenance);
	COPY_SCALAR_FIELD(errorReference);
	COPY_SCALAR_FIELD(insertCount);
	COPY_NODE_FIELD(parameter);
	COPY_SCALAR_FIELD(reestInsCnt);
...
_copyModelInfo(ModelInfo *from){
	...
	newnode->otherErrors=from->otherErrors;
	newnode->errorArray=from->errorArray;
	newnode->modelMemCtx=from->modelMemCtx;
	newnode->errorsForDropingDecision=from->errorsForDropingDecision;
	COPY_SCALAR_FIELD(upperBound);
	COPY_SCALAR_FIELD(lowerBound);
	...
	newnode->maintenance=from->maintenance;
	COPY_SCALAR_FIELD(confidenceIntervalPercentage);
...
_copyForecastExpr(ForecastExpr *from)
{
	...
	COPY_STRING_FIELD(modelName);
	COPY_STRING_FIELD(targetDateString);
 	...
 	COPY_SCALAR_FIELD(choose);
	COPY_SCALAR_FIELD(altLength);
 	COPY_SCALAR_FIELD(storeModel);
	COPY_SCALAR_FIELD(maintenancetype);
	COPY_NODE_FIELD(maintenanceParameters);
	COPY_NODE_FIELD(confidenceIntervalPercentage);
...
static FillModelGraphStmt *_copyFillModelGraph(FillModelGraphStmt *from){
	...
	newnode->maintenanceMethod = from->maintenanceMethod;
	COPY_NODE_FIELD(maintenanceParameters);
	return newnode;
}
...
copyObject(void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_RestoreStmt:
			...
			break;
		case T_SimpleReestMaintenance:
			retval= _copySimpleReestMaintenance(from);
			break;
		case T_EmptyMaintenance:
			retval= _copyEmptyMaintenance(from);
			break;
