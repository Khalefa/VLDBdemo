...
static HeapScanDesc heap_beginscan_internal(Relation relation,
						...
						bool is_bitmapscan);
static HeapScanDesc heap_beginscan_ar_internal(Relation relation,
						Snapshot snapshot,
						int nkeys, ScanKey key,
						bool allow_strat, bool allow_sync,
						bool is_bitmapscan);
 ...
 * ----------------
 */
bool PageIsNeeded (HeapScanDesc scan, ScanDirection dir, int page)
{
	Datum *values;
	bool *isNull;
	int i;
	...
	Relation r = scan->rs_rd;
	List* qual;
	TupleTableSlot *slot;
	int natts = RelationGetDescr(r)->natts;
	bool needed = false;
	int tpp = 0;
	int lastvalue = 0; /* use it latter*/

	if (r->rd_rel->relkind != RELKIND_ARRAY) return true;
	if (scan->qual == NULL) return true;
	...
	slot = scan->slot;
	qual = scan->qual;
	tpp = tuplesperpage(scan->rs_rd->rd_id);
	//elog(WARNING,"tuples per page %d from %d -%d",tpp,page * tpp,(page+1) * tpp);
	// iterate through virtual tuples if any match the qual then we need the page
	for (i = page * tpp; i< (page+1) * tpp;i+=ar_step) {
		values[0] = i;
		tuple = heap_form_tuple(r->rd_att, values, isNull);
		ExecStoreTuple(tuple, slot,  scan->rs_cbuf,   false);	
		econtext->ecxt_scantuple = slot;
		if ( ExecQual(qual, econtext, true)){
			//elog(WARNING,"The imaginary tuple is needed %d", i);
			needed = true;
	...
	pfree(isNull);
	pfree(values);
//	elog(WARNING, "Page %d is needed %d", page, needed);
	return needed;			 
...
//	elog(WARNING, "page at time");
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			page = scan->rs_startblock; /* first page */
			/* here */
			// Fast Forward to the interesting page
			for (;;) {
				pageisneeded = PageIsNeeded(scan,dir,page);
				if (pageisneeded == true) break;
				page++;
				if (page >= scan->rs_nblocks) {
					tuple->t_data = NULL;
					return;
	...
	for (;;)
	{
		...
		else
		{
			...
			for (;;) {
				...
				pageisneeded = PageIsNeeded(scan,dir,page);
		//		elog(WARNING, "page %d needed %d",page,pageisneeded);	
				if (pageisneeded == true) break;
				page++;
...
/* beginscan for an array*/
HeapScanDesc
heap_beginscan_ar(Relation relation, Snapshot snapshot,
			   int nkeys, ScanKey key)
{
	HeapScanDesc hbegin_i= heap_beginscan_internal(relation, snapshot, nkeys, key,
								   true, true, false);
	int tuples = tuplesperpage(relation->rd_id);
//	elog(WARNING, "starting at tuple %d",ar_start/tuples);	
	hbegin_i->rs_startblock=ar_start/tuples; /* this needs to be modified*/
	return hbegin_i;
...
void
heap_inplace_update_withid(Relation relation,ItemPointer otid, HeapTuple tuple)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	...
	if (oldlen != newlen || htup->t_hoff != tuple->t_data->t_hoff)
		elog(ERROR, "heap_inplace_update: wrong tuple length");
	// we can not change the first column of the relation, make sure this holds
	oldtup.t_data = (HeapTupleHeader) PageGetItem(page, lp);
	oldtup.t_len = ItemIdGetLength(lp);
	oldtup.t_self = *otid;

	valid = heap_tuple_attr_equals(RelationGetDescr(relation),1, &oldtup, tuple);
	if (valid == false) {
		LockBuffer(buffer, BUFFER_LOCK_UNLOCK); // try to remove this
		ReleaseBuffer(buffer);

		elog(ERROR, "not valid update");
