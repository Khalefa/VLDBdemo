		...
		switch (operation)
		{
			...
			case CMD_DELETE:
				...
				*/
				...
				//elog(WARNING, "ExecMain:1693  Relation kind %c ",resultRelationDesc->rd_rel->relkind);

				if (resultRelationDesc->rd_rel->relkind == RELKIND_ARRAY) {
					elog(ERROR,"Could not delete from a TimeSeries");
		   ...
		   EState *estate)
{
	...
	if (resultRelationDesc->rd_rel->relkind == RELKIND_ARRAY) {
	bool isnull;
	int last_value = lastvalue(resultRelationDesc->rd_id);
	int new_val = DatumGetInt32 (slot_getattr (slot, 1, &isnull));
	//elog(WARNING,"new value %d  last_value %d", new_val, last_value);
	if (last_value+1 != new_val) {
		ereport(ERROR, (errcode(ERRCODE_CHECK_VIOLATION),
					 errmsg("new row for relation \"%s\" violates array constraint ",
							RelationGetRelationName(resultRelationDesc))));
		}
	}
	...
	/* if array then do appropiate processing */
	if (resultRelationDesc->rd_rel->relkind == RELKIND_ARRAY) {
		int n=(BLCKSZ- sizeof(PageHeaderData))/(tuple->t_len+ sizeof(ItemIdData));
		//elog(WARNING,"Update last value");
		UpdateLastValue(resultRelationDesc->rd_id,tuple->t_len,n);
	}
...
lreplace:;
	 ...
	 */
	if (resultRelationDesc->rd_rel->relkind == RELKIND_ARRAY) {
		heap_inplace_update_withid(resultRelationDesc,tupleid, tuple);
