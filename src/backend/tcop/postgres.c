					   ...
					   Oid *paramTypes, int numParams)
{
	...
	//elog(ERROR,"%s",nodeToString(query));
...
exec_simple_query(const char *query_string)
{
	...
	if(OnecommandTag==NULL)
		OnecommandTag="UNKNOWN";

if(log_StatementTime!=-1 && strcmp(OnecommandTag,"SET"))
	{
		long		secs;
		int			usecs;
		int			msecs;
		...
		start_xact_command();
		PushActiveSnapshot(GetTransactionSnapshot());
		r=try_heap_openrv(makeRangeVar(NULL,"pg_commandtimings",-1),RowExclusiveLock);

		

		if(!r)
		{
			elog(ERROR,"no timing table found: pg_commandtimings");
		}
		...
		values[1]=Float8GetDatum(atof(msec_str));
		values[0]=Int32GetDatum(log_StatementTime);
		tuple=heap_form_tuple(RelationGetDescr(r),values,isNull);
		simple_heap_insert(r, tuple);
...
exec_execute_message(const char *portal_name, long max_rows)
{
	...
	if(log_StatementTime!=-1 && strcmp(oneCommandTag,"SET"))
	{
		...
		executeTime+=atof(msec_str);
		//PushActiveSnapshot(GetTransactionSnapshot());
		Relation r=try_heap_openrv(makeRangeVar(NULL,"pg_commandtimings",-1),RowExclusiveLock);
		HeapTuple		tuple;
		...
		values[2]=Int32GetDatum(1);
		values[1]=Float8GetDatum(executeTime);
		values[0]=Int32GetDatum(log_StatementTime);
		tuple=heap_form_tuple(RelationGetDescr(r),values,isNull);
		simple_heap_insert(r, tuple);
		CatalogUpdateIndexes(r, tuple);
		heap_freetuple(tuple);
		executeTime=0.0;
